...
use Modules\BaseModule\Enums\Files\FileType as FileTypeEnum;
use Modules\BaseModule\Enums\General\BooleanState;
use Modules\BaseModule\Services\WorkFlowService;
//use Dornica\Foundation\Localization\Localization;
...
if (!function_exists('getUploadRequirements')) {
    function getUploadRequirements(
        ...
        string  $type = 'image',
        string|Builder|null $entity = null,
        ?int    $entityId = null,
        ?string $entityFileRelation = null
    ): array
    {
        // Define type-specific settings (values are in KB)
        $typeSettings = [
            'image' => [
        ...
        // Check if file is required based on entity relationship
        $entityRequirement = null;
        if ($entity && $entityId && $entityFileRelation) {
            $record = $entity instanceof Builder
                ? $entity->find($entityId)
                : (is_string($entity) ? $entity::find($entityId) : null);

            $entityRequirement = $record?->$entityFileRelation === null ? 'required' : 'nullable';
        }

        // Apply type-specific settings
        $defaultMaxFileSize = $typeSettings[$type]['maxSize'];
        $defaultMimes = $typeSettings[$type]['mimes'] ?? $defaultMimes;

        // Initialize with defaults
        $fileTypeRequired = $defaultRequirement;
        $fileTypeMimes = $defaultMimes;
        $fileTypeMaxFileSize = $defaultMaxFileSize;

        // Override with fileType-specific settings
        if ($fileType) {
            $fileTypeRequired = $fileType->is_required->value === BooleanState::YES->value ? 'required' : 'nullable';
            $fileTypeMimes = $fileType->allowed_extensions ?? $defaultMimes;

            $fileTypeMaxFileSize = parseSize($fileType->max_size);
        }

        // Combine entity & file type "required" logic
        $fileTypeRequired = (
            ((isset($entityRequirement) && $entityRequirement === 'required') || !isset($entityRequirement))
            && $fileTypeRequired === 'required'
        ) ? 'required' : 'nullable';

        return [
            'isRequired' => $fileTypeRequired,
            'maxFileSize' => (int)$fileTypeMaxFileSize, // KB
            'mimes' => normalizeString($fileTypeMimes),
        ];
    }
}
...
if (!function_exists('checkDatesOverlap')) {
    function checkDatesOverlap(string $modelClass, $startDate, $endDate, $roleId, $ignoreId = null, $type = null): bool
    {
...
if (!function_exists('renderBadgeForProvince')) {
    function renderBadgeForProvince(string $empty_relation_text = '-'): Closure
    {
        return function ($value, $model) use ($empty_relation_text) { // NOSONAR: this function require both $value and $entity
            try {
...
if (!function_exists('booleanStateBadgeVariant')) {
    /**
     * @param $value
     * @return string
     */
    function booleanStateBadgeVariant($value, $customEnum = null): string
    {
        $comparedValue = $value == BooleanState::YES;
        if ($customEnum) {
            $comparedValue = $value == $customEnum;
        }
        return $comparedValue ? 'success' : 'danger';
    }
}

if (!function_exists('renderBadgeForRelationWithCustomColor')) {
    function renderBadgeForRelationWithCustomColor(
        string|array $relations,
        string|array $column = 'name',
        string       $empty_relation_text = '-'
    ): Closure
    {
        return function ($value, $model) use ($relations, $column, $empty_relation_text) { // NOSONAR: this function require both $value and $entity
            try {
            ...
            return "<span class='text-wrap line-height-normal badge' style='background-color: {$customColor};color: #FFF;'>" . $name . "</span>";
        };
    }
}
...
if (!function_exists('formatMinutesToHours')) {
    ...
    /**
     * Convert total minutes into a formatted string (hours and minutes).
     *
     * Examples:
     ...
     * @return string Formatted string with hours and minutes
     */
    /**
     * Convert total minutes into a formatted string (translatable).
     *
     * Examples:
     *   80  -> "1 hour and 20 minutes"
     *   60  -> "1 hour"
     ...
     *
     * @param int $minutes Total number of minutes
     * @return string Formatted string with hours and minutes (using translations)
     */
    function formatMinutesToHours($minutes)
    {
        $hours = floor($minutes / 60); // Calculate hours
        $mins  = $minutes % 60;        // Calculate remaining minutes

        if ($hours > 0 && $mins > 0) {
            return $hours . ' ' . __('basemodule::field.hours') . ' ' . __('basemodule::field.and') . ' ' . $mins . ' ' . __('basemodule::field.minutes');
        } elseif ($hours > 0) {
            return $hours . ' ' . __('basemodule::field.hours');
        } else {
            return $mins . ' ' . __('basemodule::field.minutes');
        }
    }
