...
class StatusController extends Controller
{
    ...
    public function checkIfBlogStatusExist()
    {
        $activeStatuses = BlogStatus::query()
            ->where('is_active', BooleanState::YES)
            ->get();

        $activeStartExists = $activeStatuses->where('is_start', BooleanState::YES)->first();
        $activeExpiredExists = $activeStatuses->where('is_expired', BooleanState::YES)->first();
        $activeDraftExists = $activeStatuses->where('is_draft', BooleanState::YES)->first();
        $activePublishedExists = $activeStatuses->where('is_publish', BooleanState::YES)->first();

        return Response::success(
            data: [
                'activeStartExists' => isset($activeStartExists),
                'activeExpiredExists' => isset($activeExpiredExists),
                'activeDraftExists' => isset($activeDraftExists),
                'activePublishedExists' => isset($activePublishedExists),
            ]
        );
    }
}
