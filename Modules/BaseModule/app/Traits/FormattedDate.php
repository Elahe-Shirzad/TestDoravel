...
namespace Modules\BaseModule\Traits;
...
trait FormattedDate
{
    ...
    protected function readAt(): Attribute
    {
        return Attribute::make(
            get: function () {
                if (!array_key_exists('read_at', $this->attributes) ||
                    is_null($this->attributes['read_at'])) {
                    return null;
                }

                return verta($this->attributes['read_at'])->format(jdateFormat('datetime_comma'));
            }
        );
    }
     ...
     * @return Attribute
     */
    protected function repliedAt(): Attribute
    {
        return Attribute::make(
            get: function () {
                if (!array_key_exists('replied_at', $this->attributes) ||
                    is_null($this->attributes['replied_at'])) {
                    return null;
                }

                return verta($this->attributes['replied_at'])->format(jdateFormat('datetime_comma'));
            }
        );
    }
    ...
    protected function startedAt(): Attribute
    {
        return Attribute::make(
            get: function () {
                if (!array_key_exists('started_at', $this->attributes) ||
                    is_null($this->attributes['started_at'])) {
                    return null;
                }

                return verta($this->attributes['started_at'])->format(jdateFormat('datetime_minute'));
            },
            set: function ($value) {
                if (is_null($value)) {
                    $this->attributes['started_at'] = null;
                } else {
                    $this->attributes['started_at'] = verta()->parse($value)->toCarbon()->format(jdateFormat('datetime_minute'));
                }
    ...
    protected function endAt(): Attribute
    {
        return Attribute::make(
            get: function () {
                if (!array_key_exists('end_at', $this->attributes) ||
                    is_null($this->attributes['end_at'])) {
                    return null;
                }

                return verta($this->attributes['end_at'])->format(jdateFormat('datetime_minute'));
            },
            set: function ($value) {
                if (is_null($value)) {
                    $this->attributes['end_at'] = null;
                } else {
                    $this->attributes['end_at'] = verta()->parse($value)->toCarbon()->format(jdateFormat('datetime_minute'));
                }
